...
export async function syncSteadySubscription(code: string) {
  ...
  const tokenResponse = await axios.post('https://steadyhq.com/api/v1/oauth/token', {
  ...
  const subscriptionResponse = await axios.get('https://steadyhq.com/api/v1/subscription', {
