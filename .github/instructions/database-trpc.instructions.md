...
export const postRouter = createTRPCRouter({
  getInfinite: publicProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const followingUserIds = followingOnly && ctx.session?.user.id
        ? await getFollowingUserIds(ctx.db, ctx.session.user.id)
        : [];
      
      const posts = await ctx.db.query.posts.findMany({
        where: (posts, { eq, and, lt, inArray }) => and(
...
export const growRouter = createTRPCRouter({
  create: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const userGrowCount = await ctx.db.query.grows.findMany({
...
export const notificationRouter = createTRPCRouter({
  getUnreadCount: protectedProcedure
    .query(async ({ ctx }) => {
      const count = await ctx.db.query.notifications.findMany({
...
export const plantRouter = createTRPCRouter({
  batchUpdate: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const updatedPlants = await ctx.db.update(plants)
        .set({
          ...updates,
          updatedAt: new Date(),
        })
